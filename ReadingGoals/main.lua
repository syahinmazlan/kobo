...
function ReadingGoal:_isAtZeroRemaining()
    if self:goalActive() then
        local curr, total, _sl, stable_idx, _sc = self:_getPages()
        if self.goal_type == "percentage" then
            if not (curr and total and total > 0) then return false end
            return math.max(self.goal_percentage - (curr / total) * 100, 0) == 0
        elseif self.goal_type == "stable_page" then
            if not stable_idx then return false end
            return math.max(self.goal_stable_page_idx - stable_idx, 0) == 0
        else
            if not curr then return false end
            return math.max(self.goal_page - curr, 0) == 0
        end
    end

    for _, dw in ipairs(self:_getAllActiveDailyWeekly()) do
        local read = self:_getDailyWeeklyRead(dw)
        local effective = dw.target_pages or 0
        if math.max(0, effective - read) == 0 then
            return true
        end
    end
    return false
...
function ReadingGoal:_maybeFireGoal()
    local zero_remaining = self:_isAtZeroRemaining()
    if not self:goalActive() then
        self:_trackPages()
        self:_checkDailyWeeklyReached()
        if zero_remaining and not self._last_zero_remaining then
            self:update_status_bars(-1)
        else
            self:_refreshStatusBars()
        end
        self._last_zero_remaining = zero_remaining
        return false
    end
    local curr, total, _sl, stable_idx, _sc = self:_getPages()
    self:_trackPages()
