...
function ReadingGoal:_statusBarText()
    ...
    for _, dw in ipairs(self:_getAllActiveDailyWeekly()) do
        local read = tonumber(self:_getDailyWeeklyRead(dw)) or 0
        local effective = tonumber(dw.target_pages) or 0
        ...
        if delta > 0 then
            if compact then
                ...
                if use_pct then
                    table.insert(parts, string.format("%.1f%% left", compact_value))
                else
        ...
        elseif delta == 0 then
            ...
            if compact then
                ...
                if use_pct then
                    table.insert(parts, string.format("%.1f%% over", compact_value))
                else
